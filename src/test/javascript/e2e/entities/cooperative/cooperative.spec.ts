...
describe('Cooperative e2e test', () => {
  ...
  it('should create and save Cooperatives', async () => {
    ...
    await promise.all([cooperativeUpdatePage.setCooperativeNameInput('cooperativeName')]);

    expect(await cooperativeUpdatePage.getCooperativeNameInput()).to.eq(
      'cooperativeName',
      'Expected CooperativeName value to be equals to cooperativeName'
    );
